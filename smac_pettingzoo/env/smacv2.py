...
class SMACv2Env:
    """
    SMACV2 Env with the same interfaces with SMACv1Env
    """

    def __init__(self, map_name: str, capability_config: Dict, smacv2_env_args: dict = {}):
        self.distribution_config = capability_config
        self.env_key_to_distribution_map = {}
        self.rng = np.random.default_rng()
        self._parse_distribution_config()
        self.env = SMACv2EnvCore(map_name, capability_config, **smacv2_env_args)
        assert (
            self.distribution_config.keys() == capability_config.keys()
    ...
    def _parse_distribution_config(self):
        for env_key, config in self.distribution_config.items():
            ...
            distribution = get_distribution(config["dist_type"])(config, self.rng)
    ...
    def reset(self, seed: int = None):
        ...
        return self.env.reset(seed, reset_config)
...
class SMACv2EnvCore:
    ...
    def __init__(
        ...
        ----------
        map_name : str
            The name of the SC2 map to play. The full list
        ...
        self.use_obs_instead_of_state = use_obs_instead_of_state
        assert obs_last_action == state_last_action, "Last action shoule be observed if it is part of the state."
        ...
        if self.stochastic_attack:
            self.zero_pad_stochastic_attack = not self.capability_config["attack"]["observe"]
        ...
        self.n_agents = (
            map_params["n_agents"] if not self.replace_teammates else self.capability_config["team_gen"]["n_units"]
        )
        self.n_enemies = (
            map_params["n_enemies"] if not self.replace_teammates else self.capability_config["team_gen"]["n_enemies"]
        )
    ...
    def reset(self, seed: int = None, episode_config: Dict = None):
        ...
        self.ally_start_positions = episode_config.get("ally_start_positions", {}).get("item", None)
        self.enemy_start_positions = episode_config.get("enemy_start_positions", {}).get("item", None)
    ...
    def _kill_units_below_health_level(self):
        ...
        for al_id, al_unit in self.agents.items():
            if (al_unit.health / al_unit.health_max < self.agent_health_levels[al_id]) and not self.death_tracker_ally[
                al_id
            ]:
        ...
        if action == 0:
            ...
            if self.stochastic_attack:
                ...
                if p > self.agent_attack_probabilities[a_id]:
                    if self.debug:
                        logging.debug(f"Agent {a_id} {action_name}s {target_id}, but fails")
                    return None
            action_id = actions[action_name]
            target_tag = target_unit.tag

            can_shoot = self.can_shoot(a_id, target_unit)
    ...
    def build_obs_feature_names(self):
        ...
        if self.obs_pathing_grid:
            feature_names.extend([f"pathing_grid_{n}" for n in range(self.n_obs_pathing)])
        if self.obs_terrain_height:
            feature_names.extend([f"terrain_height_{n}" for n in range(self.n_obs_height)])
        ...
        for e_id in range(self.n_enemies):
            ...
            if self.obs_all_health and self.shield_bits_enemy > 0:
                feature_names.append(f"enemy_shield_{e_id}")
            if self.unit_type_bits > 0:
                feature_names.extend([f"enemy_unit_type_{e_id}_bit_{bit}" for bit in range(self.unit_type_bits)])
        ...
        for al_id in al_ids:
            ...
            if self.unit_type_bits > 0 and (
                ...
                feature_names.extend([f"ally_unit_type_{al_id}_bit_{bit}" for bit in range(self.unit_type_bits)])
            if self.obs_last_action:
                feature_names.extend([f"ally_last_action_{al_id}_action_{action}" for action in range(self.n_actions)])
        ...
        if self.unit_type_bits > 0:
            feature_names.extend([f"own_unit_type_bit_{bit}" for bit in range(self.unit_type_bits)])
    ...
    def can_shoot(self, agent_id, t_unit) -> bool:
        ...
        can_shoot = (
            ...
            else self.is_position_in_cone(agent_id, t_unit.pos, range="shoot_range")
        )
        return can_shoot
    ...
    def get_obs_agent(self, agent_id, fully_observable=False):
        ...
        if unit.health > 0 and self.obs_starcraft:  # otherwise dead, return all zeros
            ...
            for e_id, e_unit in self.enemies.items():
                ...
                enemy_visible = self.is_position_in_cone(agent_id, e_unit.pos) if self.conic_fov else dist < sight_range
                if enemy_visible:
                ...
                if (enemy_visible and e_unit.health > 0) or (
                    ...
                    show_enemy = (self.mask_enemies and not self.enemy_mask[agent_id][e_id]) or not self.mask_enemies
                    ind = 4
    ...
    def get_global_state(self, agent_id):
        if self.use_state_agent:
            global_state = self.get_state_agent(agent_id)
        else:
    ...
    def get_obs(self):
        ...
        during decentralised execution.
        """
        agents_obs = [self.get_obs_agent(i, fully_observable=self.fully_observable) for i in range(self.n_agents)]
        return agents_obs
    ...
    def get_obs_ally_capability_size(self):
        ...
        if self.stochastic_attack and (self.zero_pad_stochastic_attack or self.observe_attack_probs):
            cap_feats += 1
        if self.stochastic_health and (self.observe_teammate_health or self.zero_pad_health):
            cap_feats += 1
    ...
    def only_medivac_left(self, ally):
        ...
        if ally:
            ...
            units_alive = [a for a in self.enemies.values() if (a.health > 0 and a.unit_type != Terran.Medivac)]
            if len(units_alive) == 0:
                return True
            return False
