...
class AllTeamsDistribution(Distribution):
    def __init__(self, config, rng):
        ...
        self.env_key = config["env_key"]
        self.combinations = list(combinations_with_replacement(self.units, self.n_units))
...
class WeightedTeamsDistribution(Distribution):
    ...
    def _gen_team(self, n_units: int, use_exceptions: bool):
        team = []
        while not team or (all(member in self.exceptions for member in team) and use_exceptions):
    ...
    def generate(self) -> Dict[str, Dict[str, Any]]:
        ...
        if self.n_enemies > self.n_units:
            extra_enemies = self._gen_team(self.n_enemies - self.n_units, use_exceptions=True)
...
class ReflectPositionDistribution(Distribution):
    ...
    def __init__(self, config, rng):
        ...
        self.pos_generator = PerAgentUniformDistribution(config_copy, rng)
        if self.n_enemies > self.n_units:
            ...
            self.enemy_pos_generator = PerAgentUniformDistribution(enemy_config_copy, rng)
...
class SurroundedPositionDistribution(Distribution):
    ...
    def generate(self) -> Dict[str, Dict[str, Any]]:
        ...
        group_membership = self.rng.multinomial(self.n_enemies, np.ones(n_groups) / n_groups)
        # decide on the distance along the diagonal for each group
        group_position = self.rng.uniform(size=(n_groups,))
        group_diagonals = self.rng.choice(np.array(range(4)), size=(n_groups,), replace=False)
        ...
        for i in range(n_groups):
            t = group_position[i]
            enemy_position[unit_index : unit_index + group_membership[i], :] = diagonal_to_centre_point[
                group_diagonals[i]
            ] * t + diagonal_to_point_map[group_diagonals[i]] * (1 - t)
...
class SurroundedAndReflectPositionDistribution(Distribution):
    def __init__(self, config, rng):
        ...
        self.surrounded_distribution = SurroundedPositionDistribution(config, rng)
        self.reflect_distribution = ReflectPositionDistribution(config, rng)
        self.rng = rng
...
register_distribution("surrounded_and_reflect", SurroundedAndReflectPositionDistribution)
